...
public class DialogueClientAndServer1 {

	// To run this example DialogueClientAndServer2 must be running.
	...
	public static void main(String[] args) {
		...
		try {
			...
			// Create the dialogue and start listen for messages on port 8098
			...
			dialogue.addDestination(DialogueFactory.createIPDestination("Dest2", 8099));
				...
				public void messageRecieved(DialogueServerMessage dialogMessage) {
					try {
						...
						if (dialogMessage.responseRequested())
							dialogMessage.respond("Response from destination 1: " + dialogMessage.getString());
			...
			DialogueResponse resp = dialogue.sendResponseMessage("Dest2", "Message1 from destination 1", 5000);
			// Print the response
			try {
			...
			// giving up
			resp = dialogue.sendResponseMessage("Dest2", "Message2 from destination 1", 2000);
			// Print the response
			try {
				System.out.println(resp.getString() == null ? "No response received" : resp.getString());
			} catch (Exception e) {
